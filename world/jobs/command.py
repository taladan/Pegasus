...
class CmdBuckets(MuxCommand):
    ...
    def _check_actions(self, bucket, target):
        """build list of actions target can access on given bucket"""
        actions = []
        for action in self.valid_actions:
            if bucket.has_access(action, target):
                actions.append(action[0])
        return actions

    def _check(self, target):
        """displays bucket actions that a player has access to"""
        buckets = []
        actions = []
        for bucket in self.buckets:
            actions = self._check_actions(bucket, target)
            if actions:
                buckets.append(bucket)

        if actions:
            ret = self._info_table(self.character, buckets, actions)
        else:
            ret = _SUCC_PRE + "%s can not perform any actions on any bucket." % self.character

        self.caller.msg(ret)

    def _can_access(self, action, caller):
        """lock validation falls through to bucket.has_access(action, caller)"""
        locks = caller.locks.check_lockstring
        if locks(caller, "dummy:perm(Admin)") or locks(caller, "dummy:perm(BucketCreator)"):
            return True
        elif self.bucket.has_access(action, caller):
            return True
        else:
            return False
    ...
    def _create(self, caller):
        ...
        # Exists?
        if self.bucket:
            caller.msg(_ERROR_PRE + "Bucket: |w%s|n already exists." % self.bucket_name)
            return
        ...
        try:
            ...
            caller.msg(_SUCC_PRE + "Bucket: |w%s|n has been created." % self.bucket_name)
        except AttributeError:
            self.caller.msg(_ERROR_PRE + "A description must be provided for the bucket.")

    def _delete(self, caller):
        """Deletes bucket.  Bucket may not have any jobs inside of bucket"""
        self.caller.msg(_TEST_PRE + "Entering Delete")
        if self.bucket is not None:
            if not self.bucket.has_jobs():
                # Todo: check bucket for jobs first
                caller.msg(_SUCC_PRE + "Bucket: |w%s|n deleted." % self.bucket_name)
                ev.search_channel(self.bucket_name).first().delete()
            else:
                caller.msg(_ERROR_PRE + "Cannot delete Bucket: |w%s|n, jobs are associated with that bucket"
                           % self.bucket_name)
        else:
            caller.msg(_ERROR_PRE + "Bucket: |w%s|n does not exist." % self.bucket_name)
            return

    def _info(self, bucket, caller):
        """Display info for particular bucket
         :param bucket: """
        caller.msg(self._info_table(caller, self.buckets, self.valid_actions))

    def _info_table(self, caller, buckets, actions):
        """build the table for the _check and return it"""
        ret = evtable.EvTable("Buckets %s has access to:" % caller, "Actions available to: %s" % caller,
                              header=True,
                              border="table",
                              header_line_char="-",
                              width=95,
        ...
        for bucket in buckets:
            try:
                bucket_action_list = bucket.db.info(caller)
                ret.add_row(buck.key, ', '.join(bucket_action_list))
            except TypeError:
                ret = _ERROR_PRE + "No buckets exist."
    ...
    def func(self):
        ...
        if self.switch and self._can_access(self.switch, self.caller):
            ...
            msg = self._info(self.buckets, self.caller)
            self.caller.msg(msg)
